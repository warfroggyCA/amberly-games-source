...
vi.mock("../src/server/database", () => ({
  getGymRepository: () => ({ read: mock.read, append: mock.append }),
}));
...
import { GET, POST } from "../src/app/api/family/gym/route";
...
beforeEach(() => {
  ...
  mock.append.mockResolvedValue({ eventId: "receipt" });
});
afterEach(() => vi.unstubAllEnvs());
it("requires the matching signed-in identity and returns private uncached data", async () => {
  const bad = await GET(new Request(origin + "/api/family/gym"));
  expect(bad.status).toBe(401);
  expect(mock.read).not.toHaveBeenCalled();
  const good = await GET(
    new Request(origin + "/api/family/gym", {
...
it("rejects cross-origin and oversized writes before repository work", async () => {
  for (const [body, requestOrigin, status] of [
    ...
    [JSON.stringify({ large: "a".repeat(540000) }), origin, 413],
  ] as const) {
