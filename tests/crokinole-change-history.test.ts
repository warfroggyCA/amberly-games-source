...
it("tracks v2 amendments while ended early and the explicit resumption", () => {
  const f = fixture();
  f.record(40, 30);
  f.record(20, 10);
  f.apply({ type: "end_early", reason: "Dinner" });
  f.apply({
    type: "correct_round_v2",
    roundId: "round-0",
    entries: entries(50, 20),
    excludedRoundIds: [],
    reason: "Correct the saved totals",
  });
  f.apply({ type: "undo_round_v2", reason: "Duplicate round" });
  const resumed = f.apply({ type: "resume", reason: "Ready to finish" });
  const changes = crokinoleChangeHistory(resumed);
  expect(changes.map((change) => change.afterStatus)).toEqual([
    "ended_early",
    "ended_early",
    "ended_early",
    "active",
  ]);
  expect(changes[1].roundNumber).toBe(1);
  expect(changes[1].beforeEntries).toEqual(entries(40, 30));
  expect(changes[1].afterEntries).toEqual(entries(50, 20));
  expect(changes[2].roundNumber).toBe(2);
  expect(changes[2].afterTotals).toEqual({ a: 50, b: 20 });
  expect(changes[3].beforeStatus).toBe("ended_early");
  expect(changes[3].beforeEntries).toBeNull();
  expect(changes[3].afterTotals).toEqual(resumed.totals);
});
