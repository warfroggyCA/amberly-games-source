...
export function createCrokinoleRepository(
  ...
  async function checked(tx: Tx, familyId: string, gameId: string) {
    ...
    const game = checkedCrokinoleState(
      row.definition,
  ...
  async function mutate(
    ...
    const staleProjection = Symbol("stale projection");
    const gameIds =
      "gameId" in op
        ? [op.gameId]
        : op.type === "create-game" && isCrokinoleDefinition(op.definition)
          ? [op.definition.id]
          : [];
    if (op.type === "rematch") gameIds.push(op.newGameId);
    for (let attempt = 0; attempt < 3; attempt++) {
      const snapshots = await transaction(
        ...
        false,
        async (tx) => {
          const games = new Map<string, Awaited<ReturnType<typeof checked>>>();
          for (const gameId of gameIds) {
            if (!id(gameId)) fail("INVALID_GAME", "Invalid game reference.");
            const [exists] =
              await tx`select game_id from scrabble.crokinole_games where family_id=${familyId}::uuid and game_id=${gameId} and not removed`;
            if (exists) games.set(gameId, await checked(tx, familyId, gameId));
          }
          return games;
        },
      );
      async function currentGame(tx: Tx, familyId: string, gameId: string) {
        const [row] =
          await tx`select * from scrabble.crokinole_games where family_id=${familyId}::uuid and game_id=${gameId} and not removed`;
        if (!row) fail("GAME_NOT_FOUND", "This game is not available.", 404);
        ...
        if (
          !snapshot ||
          row.revision !== snapshot.row.revision ||
          !isDeepStrictEqual(row.state, snapshot.row.state) ||
          !isDeepStrictEqual(row.definition, snapshot.row.definition)
        )
          throw staleProjection;
        // Read current ownership under the lock; only the verified projection is reused.
        return { row, game: snapshot.game };
      }
      try {
        return await transaction(actor, familyId, true, async (tx, who) => {
          const [prior] =
          ...
          if (prior) {
            ...
            if ("gameId" in op && !response.removedGameId) {
              const current = await currentGame(tx, familyId, op.gameId);
          ...
          if (op.type === "save-defaults") {
            ...
            if (
              ...
              op.colours.some(
                (c) =>
                  c.isDefault &&
                  !DEFAULT_PIECE_COLOURS.some((d) => d.id === c.id),
              )
            )
            ...
            const { game: source } = await currentGame(tx, familyId, op.gameId);
            if (
              !id(op.newGameId) ||
              !revision(op.expectedRevision) ||
            ...
            const { row, game } = await currentGame(tx, familyId, op.gameId);
            const before = game;
            ...
            if (op.type === "command") {
              ...
              if (
                ...
                (op.command.type === "correct_round" ||
                  op.command.type === "undo_round")
              )
                fail(
                  "CLIENT_UPDATE_REQUIRED",
                  "Reload before amending an ended-early match. Your entry is still available.",
                  409,
                );
              ...
              const cleared: CrokinoleDraft = {
                ...
                generation: row.generation,
                values:
                  op.command.type === "resume"
                    ? (d?.values ?? {})
                    : op.command.type === "undo_round" ||
                        op.command.type === "undo_round_v2"
                      ? Object.fromEntries(
                          game.rounds
                            .at(-1)!
                            .entries.map((e) => [
              ...
              if (
                ...
                fail(
                  "DRAFT_CONFLICT",
                  "Another device saved a newer draft.",
                  409,
                );
        ...
        if (error !== staleProjection) throw error;
      }
    }
    return fail(
      "REVISION_CONFLICT",
      "The match changed. Review your retained entry before retrying.",
      409,
    );
  }
